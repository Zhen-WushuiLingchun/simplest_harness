...
describe("ContextCompactor", () => {
  it("writes a validated artifact atomically and retains the recent tail verbatim", async () => {
    ...
    expect(result.artifact.recentTail).toEqual([
      { role: "assistant", text: "one" },
      { role: "user", text: "exact tail" },
    ]);
    expect(JSON.parse(await readFile(result.artifactPath, "utf8"))).toEqual(
      result.artifact,
    );
  });
  ...
  it("rejects an incorrect source boundary and a summarizer failure", async () => {
    ...
    expect(
      validateCompactionCandidate(
        { ...validCandidate(ledger), source: { ...source, throughSeq: 11 } },
        ledger,
        source,
      ),
