...
export function calculateThresholds(
  ...
  const reservedOutputTokens = config.reservedOutputTokens ?? modelMaxOutputTokens ?? 4_096;
  ...
  if (maxInputTokens === undefined) return { capacityKnown: false, reservedOutputTokens };
  if (!Number.isInteger(maxInputTokens) || maxInputTokens <= reservedOutputTokens) {
  ...
  const softTriggerTokens = config.triggerTokens ?? Math.floor(usableInputTokens * config.triggerRatio);
  const hardTriggerTokens = Math.floor(usableInputTokens * config.hardRatio);
  if (!Number.isInteger(softTriggerTokens) || softTriggerTokens <= 0 || softTriggerTokens >= hardTriggerTokens) {
    throw new Error("soft compaction trigger must be positive and below the hard trigger");
  }
  return { capacityKnown: true, reservedOutputTokens, usableInputTokens, softTriggerTokens, hardTriggerTokens };
}
...
export function decideCompaction(input: {
  ...
  const thresholds = calculateThresholds(input.config, input.maxInputTokens, input.modelMaxOutputTokens);
  if (input.config.mode === "off") return { ...thresholds, action: "skip", reason: "disabled" };
  if (input.modelRequested && input.config.allowModelEarly) {
    return { ...thresholds, action: "compact", reason: "model_requested" };
  }
  if (input.config.mode === "manual") return { ...thresholds, action: "continue", reason: "manual_mode" };
  if (!thresholds.capacityKnown) {
    return { ...thresholds, action: "skip", reason: "unknown_model_capacity" };
  }
  const projected = input.observedInputTokens + (input.pendingEstimatedTokens ?? 0);
